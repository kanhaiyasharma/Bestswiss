...
class WbsSettingsHtmlTools
{
    ...
    public function validateRangeHtml($key)
    {
        ...
        $input = @$_POST[$this->getPostKey($key)];
    ...
    {
        $inputNamePrefix = $this->getPostKey($key);
    ...
    {
        return self::receiveWeightRate(@$_POST[$this->getPostKey($key)]);
    }

    public function generateShippingClassesHtml($key, $data)
    {
        $prefix = $this->getPostKey($key);
    ...
    public function validateShippingClasses($key)
    {
        ...
        foreach ($rows as $i => $row) {
            $rates->add(new WbsBucketRate(
                @$row['class'],
                self::receiveDecimal(@$row['fee']),
                WbsProgressiveRate::fromArray(self::receiveWeightRate(@$row['weight_rate']))
            ));
        }
    ...
    public function premiumPromotionHtml($feature = 'The feature')
    {
        return '
            <p>
                '.esc_html($feature).' is available in the
                <a href="http://bit.ly/wbscc" target="_blank">Pro version</a>.
                Try out a <a href="http://bit.ly/wbsccdemo" target="_blank">live demo</a>.
            </p>';
    }
    ...
    public function trsPromotionHtml()
    {
        return '
            <p>
                In case you need a more flexbile shipping solution take a look at our <a href="http://bit.ly/wbs2trs" target="_blank">advanced shipping plugin</a>.
            </p>
        ';
    }
    ...
    {
        return method_exists($this->settings, 'get_field_key')
            ? $this->settings->get_field_key($key)
            : "{$this->settings->plugin_id}{$this->settings->id}_{$key}";
    }

    private static function receiveDecimal($value, $defaultValue = null)
    {
        if (isset($value)) {
    ...
    private static function receiveWeightRate($input)
    {
        $input = (array)$input;

        return array(
            'cost' => self::receiveDecimal(@$input['cost']),
            'step' => self::receiveDecimal(@$input['step']),
            'skip' => self::receiveDecimal(@$input['skip']),
        );
    }
}